...
export default function Login() {
    const [email, setEmail] = useState('') // Create state variables for email and password
    ...
    const handleLogin = async (e) => {
        ...
        const { error } = await supabase.auth.signInWithPassword({ email, password }) // Call the signInWithPassword method from the supabase client
        if (error) setError(error.message) // If there is an error, set the error state variable to the error message 
        else navigate('/') // If there is no error, navigate to the home page 
    }

    const handleSocialSignIn = async (provider) => {
        setError('') // Clear any previous errors
        const { error } = await supabase.auth.signInWithOAuth({
            provider, // Pass the provider (google or discord)
            options: {
                redirectTo: window.location.origin, // Redirect to the home page after successful login
            }
        })
        if (error) setError(error.message) // Handle errors
    }
...
// Going to style this with tailwind 